...
public class UiAutomatorView extends Composite {
	...
	public UiAutomatorView(Composite parent, int style) {
		...
		//MyMenu
		final org.eclipse.swt.widgets.Menu menu = new org.eclipse.swt.widgets.Menu(mScreenshotCanvas);
		final org.eclipse.swt.widgets.MenuItem mi1 = new org.eclipse.swt.widgets.MenuItem(menu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi2 = new org.eclipse.swt.widgets.MenuItem(menu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi3 = new org.eclipse.swt.widgets.MenuItem(menu, SWT.Selection);
		final org.eclipse.swt.widgets.MenuItem mi4 = new org.eclipse.swt.widgets.MenuItem(menu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi5 = new org.eclipse.swt.widgets.MenuItem(menu, SWT.CASCADE);
		org.eclipse.swt.widgets.Menu optionmenu = new org.eclipse.swt.widgets.Menu(menu);
		org.eclipse.swt.widgets.MenuItem mi51 = new org.eclipse.swt.widgets.MenuItem(optionmenu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi52 = new org.eclipse.swt.widgets.MenuItem(optionmenu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi53 = new org.eclipse.swt.widgets.MenuItem(optionmenu, SWT.Selection);
		org.eclipse.swt.widgets.MenuItem mi54 = new org.eclipse.swt.widgets.MenuItem(optionmenu, SWT.Selection);
		mi5.setMenu(optionmenu);
		mi1.setText("点击控件");
		mi2.setText("添加等待时间");
		mi3.setText("截取屏幕");
		mi4.setText("输入文本");
		...
		mScreenshotCanvas.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e){
				if(e.button == 3){
					mi4.setEnabled(false);
					for(Object e1 : mModel.getSelectedNode().getAttributesArray()){
						if(((AttributePair)e1).key=="class"&&((AttributePair)e1).value.endsWith("EditText")){
							mi4.setEnabled(true);
						}
					}
		...
		/**
		 * @author wangxiaochun1
		 * MyDef  
		 * date :  2015-07-28
		 */
		//bottom-right base
		Composite bottomRightBase = new Composite(rightSash, SWT.BORDER);
		bottomRightBase.setLayout(new FillLayout());
		//Script field
		scriptArea = new Text(bottomRightBase, SWT.SCROLL_PAGE);
		...
		mi1.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				String index = "0";
				for(Object e : mModel.getSelectedNode().getAttributesArray()){
					if(((AttributePair)e).key=="index"){
						index = ((AttributePair)e).value;
					}
					if(((AttributePair)e).key=="text"&&((AttributePair)e).value!=null&&((AttributePair)e).value!=""){
						scriptArea.append("//点击文本"+"\""+((AttributePair)e).value+"\"\n");
						scriptArea.append("solo.clickOnText(\""+((AttributePair)e).value+"\");" + "\n");
						break;
					}
					else if(((AttributePair)e).key=="resource-id"&&((AttributePair)e).value!=null&&((AttributePair)e).value!=""){
						scriptArea.append("//点击View"+"\""+((AttributePair)e).value+"\"\n");
						scriptArea.append("solo.clickOnView(solo.getView(\""+((AttributePair)e).value.substring(((AttributePair)e).value.indexOf(":id/")+4)+"\"));" + "\n");
						break;
					}
					else if(((AttributePair)e).key=="class"&&((AttributePair)e).value.endsWith("RadioButton")){
						scriptArea.append("//点击RadioButton"+"\""+index+"\"\n");
						scriptArea.append("solo.clickOnRadioButton(" + index +");" + "\n");
						break;
					}
					else if(((AttributePair)e).key=="class"&&((AttributePair)e).value.endsWith("ImageView")){
						scriptArea.append("//点击图片"+"\""+index+"\"\n");
						scriptArea.append("solo.clickOnImage(" + index +");" + "\n");
						break;
					}
				}
			}
		...
		mi2.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0) {
				InputDialog ipd = new InputDialog(getShell(), "添加等待时间", "请设置需要等待的时长(秒)", "1",null);
				ipd.open();
				scriptArea.append("//添加等待时间"+"\""+ipd.getValue()+"s\"\n");
				scriptArea.append("solo.sleep(" + (int)(Double.parseDouble(ipd.getValue())*1000) +");" + "\n");
			}
		});
		mi3.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				// TODO Auto-generated method stub
				InputDialog ipd2 = new InputDialog(getShell(), "截取屏幕", "请输入截图的名称.(推荐用 JD_YouFunction_Page_Number)", "JD_YouFunction_Page_Number",null);
				ipd2.open();
				scriptArea.append("//截图"+"\""+ipd2.getValue()+"\"\n");
				scriptArea.append("solo.takeScreenshot(\"" +ipd2.getValue() +"\");" + "\n");
			}
		...
		mi4.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				// TODO Auto-generated method stub
				String index = "0";
				InputDialog ipdEditText = new InputDialog(getShell(), "输入文本", "请输入文本(没有空格)", "jdtest",null);
				ipdEditText.open();
				for(Object e : mModel.getSelectedNode().getAttributesArray()){
					if(((AttributePair)e).key=="index"){
						index = ((AttributePair)e).value;
					...
					if(((AttributePair)e).key=="text"&&((AttributePair)e).value!=null&&((AttributePair)e).value!=""){
						scriptArea.append("//输入文本"+"\""+ipdEditText.getValue()+"\"\n");
						scriptArea.append("solo.enterText(solo.getEditText(\""+((AttributePair)e).value+"\"),\"" + ipdEditText.getValue() +"\");\n");
						break;
					}
					else if(((AttributePair)e).key=="text"&&((AttributePair)e).value==""){
						scriptArea.append("//输入文本"+"\""+ipdEditText.getValue()+"\"\n");
						scriptArea.append("solo.enterText(" + index + ",\"" + ipdEditText.getValue() +"\");\n");
						break;
					}
				}
			}
		...
		mi51.addSelectionListener(new SelectionListener() {

			@Override
			public void widgetSelected(SelectionEvent arg0) {
				// TODO Auto-generated method stub
				scriptArea.append("//向上滑动屏幕\n");
				scriptArea.append("solo.scrollToSide(solo.UP);\n");
			}
		...
		mi52.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0){
				scriptArea.append("//向下滑动屏幕\n");
				scriptArea.append("solo.scrollToSide(solo.DOWN);\n");
			}
		});
		mi53.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0){
				scriptArea.append("//向左滑动屏幕\n");
				scriptArea.append("solo.scrollToSide(solo.LEFT);\n");
			}
		});
		mi54.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0){
				scriptArea.append("//向右滑动屏幕\n");
				scriptArea.append("solo.scrollToSide(solo.RIGHT);\n");
			}
		});
